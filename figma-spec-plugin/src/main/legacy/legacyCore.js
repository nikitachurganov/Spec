...
import { AnatomyGenerator } from '../anatomy/anatomyGenerator';
...
async function getNodeByIdSafe(id) {
  return getNodeByIdSafeAsync(id);
}
...
async function createContainersSection(spec, root, designTokens, sections) {
  ...
  if (!spec.containers || spec.containers.length === 0) {
    var emptyMode =
      sections.specSelectedLayerPaths && sections.specSelectedLayerPaths.length > 0
        ? 'manual'
        : 'auto';
    var specEmpty = await createSpecContainersEmptyState(emptyMode);
    section.appendChild(specEmpty);
    stretchInParent(specEmpty);
    return section;
  }
...
async function buildSpecObject(root, options) {
  ...
  if (containers.length === 0) {
    if (options.selectedLayerPaths && options.selectedLayerPaths.length > 0) {
      warnings.push('Не найдено слоёв для Spec по выбранным настройкам.');
      console.warn('[Spec] No containers matched specSelectedLayerPaths.');
    } else {
